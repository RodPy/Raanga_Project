...
from .forms import RegistrationsForms,UserProfileForm,UserForm
...
def register (request):
    form = RegistrationsForms
    if request.method == 'POST':
        form= RegistrationsForms(request.POST)
        if form.is_valid():
            ...
            mail_subjet = "Favor activa tu cuenta en RAANGA"
            ...
            to_email = email
            send_email = EmailMessage(mail_subjet, body, to=[to_email])
            send_email.send()
        ...
        if user is not None:
            try:
                ...
                if is_cart_item_exists:
                    ...
                    product_variation=[]
                    """
                    This code appears to be a function that updates the user's cart with new items. 
                    """
                    ...
                    for item in cart_item:
                        ...
                        ex_variations_list.append(list(existing_variation))
                        id.append(item.id)

                    for pr in product_variation:
                        if pr in ex_variations_list:
                            index= ex_variations_list.index(pr)
                            item_id= id[index]
            ...
            messages.success(request, 'Has inciada sesion exitosamente')
...
def forgotPassword (request):
    if request.method == "POST":
        ...
        if Account.objects.filter(email=email).exists():
            ...
            messages.error(request,"El correo ingresado no se encuentra en la base de datos ")
